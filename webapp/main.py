...
from HTML_MD_Components import noticeBoardMarkDown, bannerHTML, emailHTML
#, discordHTML
...
with gr.Blocks(title="SmartEye👀")as io:
#, css="#discord {text-align: center} #submit {background-color: #FF8C00} #advertisment {text-align: center;} #email {height:120%; background-color: LightSeaGreen} #blank {margin:150px} #code_feedback { margin-left:-0.3em;color:gray;text-align: center;margin-bottom:-100%;padding-bottom:-100%}") as io:
    #with gr.Row():
    ...
    with gr.Row():
        ...
        with gr.Column(scale=0.8):
            gr.Markdown('<h1 style="text-align: center;">SmartEye <a style="text-decoration:none</a></h1>')
                        # href="https://github.com/BurhanUlTayyab/GPTZero">(Code)</a></h1>')
        #with gr.Column(scale=0.1, elem_id="discord"):
           # gr.HTML(discordHTML, visible=True)
    with gr.Row():
        gr.Markdown("Use SmartEye to determine if the text is written by AI or Human.")
...
if __name__ == "__main__":
    uvicorn.run("main:app", host="127.0.0.1", port=80, reload=True, forwarded_allow_ips="*", proxy_headers=True)
